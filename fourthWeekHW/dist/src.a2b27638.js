    ...
    if (serie.isWatched) {
      ...
      if (!this.lastSerie) {
        ...
        this.series.forEach(function (item) {
          var serieDate = new Date(serie.finishedDate);
          var itemDate = new Date(item.finishedDate);

          if (serieDate > itemDate) {
            _this.lastSerie = serie;
          }
        });
      }
    } else {
  ...
  this.finishSerie = function () {
    ...
    this.series.forEach(function (serie) {
      if (serie.isCurrent) {
        ...
        _this2.lastSerie = serie;
      }
    });
    this.currentSerie = this.nextSerie;
    this.currentSerie.isCurrent = true;
    series.forEach(function (serie) {
      if (!serie.isCurrent && serie.isWatched) {
        _this2.nextSerie = serie;
        return;
      }
    });
    this.numberOfWatched++;
    this.numberOfUnWatched--; // find and update currently watching serie in "this.series" array
